...
class McpHttpServer {
  ...
  async handleRequest(req, res) {
    ...
    console.log(`[HTTP ${new Date().toISOString()}] ${method} ${pathname}${parsedUrl.search} | Accept: ${acceptHeader} | Host: ${host} | Proto: ${proto} | Prefix: ${prefix}`);
    ...
    if ((pathname === "/openapi.json" || pathname === "/swagger.json") && method === "GET") {
      return this.sendJson(res, 200, this.generateOpenApiSpec(host, proto, prefix));
    }
    ...
    if ((pathname === "/.well-known/ai-plugin.json" || pathname === "/manifest.json") && method === "GET") {
      return this.sendJson(res, 200, {
        ...
        auth: { type: "none" },
        api: { type: "openapi", url: `${proto}://${host}${prefix}/openapi.json` },
        logo_url: `${proto}://${host}${prefix}/download/logo.png`,
    ...
    if (pathname.startsWith("/tools/") && method === "POST") {
      ...
      try {
        ...
        this.injectDownloadUrls(result, host, proto, prefix);
    ...
    if (isSseRequest && method === "GET") {
      ...
      if (pathname === "/sse") {
        const endpointUri = `${proto}://${host}${prefix}/message?sessionId=${sessionId}`;
    ...
    if (pathname === "/message" && method === "POST") {
      ...
      const rpcResponse = await this.dispatchRpc(body, host, proto, prefix);
    ...
    if (pathname === "/health" || pathname === "/" || pathname === "/mcp") {
      return this.sendJson(res, 200, {
        ...
        endpoints: {
          mcpSse: `${proto}://${host}${prefix}/sse`,
          mcpRpc: `${proto}://${host}${prefix}/mcp`,
          openApi: `${proto}://${host}${prefix}/openapi.json`,
          pluginManifest: `${proto}://${host}${prefix}/.well-known/ai-plugin.json`,
          download: `${proto}://${host}${prefix}/download/`
        },
  ...
  async dispatchRpc(req, host, proto, prefix = "") {
    if (Array.isArray(req)) {
      const responses = [];
      for (const singleReq of req) {
        const resp = await this.dispatchSingleRpc(singleReq, host, proto, prefix);
        if (resp !== null) responses.push(resp);
      }
      return responses;
    }
    return this.dispatchSingleRpc(req, host, proto, prefix);
  }

  async dispatchSingleRpc(req, host, proto, prefix = "") {
  ...
  injectDownloadUrls(result, host, proto, prefix = "") {
    if (result && result.files && result.files.stlPath) {
      ...
      result.downloadUrls = {
        stl: `${proto}://${host}${prefix}/download/${stlBase}`,
        scad: `${proto}://${host}${prefix}/download/${scadBase}`
      };
    }
  }

  generateOpenApiSpec(host, proto, prefix = "") {
    const paths = {};
