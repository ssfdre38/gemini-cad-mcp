...
class McpHttpServer {
  ...
  async handleRequest(req, res) {
    ...
    if (isSseRequest && method === "GET") {
      ...
      // Only send legacy 'endpoint' event for legacy SSE (/sse or /)
      if (pathname === "/sse" || pathname === "/") {
        ...
        console.log(`[StreamableHTTP SSE] Initialized stream for session ${sessionId} on ${pathname}`);
      }
      return;
    }
    ...
    if ((pathname === "/mcp" || pathname === "/rpc" || pathname === "/") && method === "POST") {
      ...
      // Push response to SSE stream if client is listening on SSE with this session
      if (sessionId && this.sessions.has(sessionId) && rpcResponse !== null) {
        const session = this.sessions.get(sessionId);
        try {
          session.res.write(`event: message\ndata: ${JSON.stringify(rpcResponse)}\n\n`);
