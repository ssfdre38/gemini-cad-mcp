...
class McpHttpServer {
  ...
  async handleRequest(req, res) {
    ...
    console.log(`[HTTP ${new Date().toISOString()}] ${method} ${pathname} | Accept: ${acceptHeader} | Host: ${host} | Proto: ${proto}`);

    // 1. Full CORS Preflight
    res.setHeader("Access-Control-Allow-Origin", "*");
    res.setHeader("Access-Control-Allow-Methods", "GET, POST, OPTIONS, HEAD");
    res.setHeader("Access-Control-Allow-Headers", "Content-Type, Authorization, X-Requested-With, Baggage, Sentry-Trace, Accept, x-goog-api-client, Mcp-Session-Id, mcp-session-id");
    ...
    // 5. MCP SSE Stream Endpoint (/sse, or / with text/event-stream)
    ...
    if (isSseRequest && method === "GET") {
      ...
      // Send initial endpoint notification with absolute URI
      const endpointUri = `${proto}://${host}/message?sessionId=${sessionId}`;
      res.write(`: connected\n\n`);
      res.write(`event: endpoint\ndata: ${endpointUri}\n\n`);
      console.log(`[SSE] Initialized stream for session ${sessionId} -> ${endpointUri}`);
      return;
    }

    // 6. MCP Message POST Endpoint (/message)
    if (pathname === "/message" && method === "POST") {
      const sessionId = parsedUrl.searchParams.get("sessionId") || req.headers["mcp-session-id"] || crypto.randomUUID();
    ...
    // 7. Direct JSON-RPC Endpoint (/mcp or / or /rpc)
